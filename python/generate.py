...
def generate_dataset(filename, dir_name, max_priority):
    path = "../runtime_dataset/{}/".format(dir_name)
    if not os.path.exists(path):
        os.makedirs(path)
    path = "../runtime_dataset/records/"
    if not os.path.exists(path):
        os.makedirs(path)
    f = open(filename, 'r')
    ...
    f_write = open("../runtime_dataset/{}/00_{}.txt".format(dir_name, max_priority), 'w')
    ...
    for pkt in tqdm(datas):
        ...
        f_write.write(row)
    f_write.close()
    #"./runtime_data/{}/00_{}.txt".format(dir_name, max_priority)
    print("Successfully generated a uniformly distributed priority dataset {}.".format("../runtime_dataset/{}/00_{}.txt".format(dir_name, max_priority)))
    real_spreads_file = open("../runtime_dataset/records/real_spreads_{}.pickle".format(dir_name),'wb')
    pickle.dump(real_spreads, real_spreads_file)
    real_spreads_file.close()
    real_flow_priority_file = open("../runtime_dataset/records/real_map_{}.pickle".format(dir_name), 'wb')
    ...
    for i in range(flow_len):
        for j in range(1, len(priority_rank_lst)):
            if i < priority_rank_lst[j]:
                ...
                break
    filename = "../runtime_dataset/{}/00_zipf_gamma={}_alpha={}.txt".format(dir_name, gamma, alpha)
    f = open(filename, 'w')
...
def get_opt_caida_2016(M, n):
    b = int(math.log2((n))) + 1
    filename1 = "../runtime_dataset/records/real_spreads_caida_2016.pickle"
    filename2 = "../runtime_dataset/records/real_map_caida_2016.pickle"
...
def get_opt_caida_2019(M, n):
    b = int(math.log2((n))) + 1
    filename1 = "../runtime_dataset/records/real_spreads_caida_2019.pickle"
    filename2 = "../runtime_dataset/records/real_map_caida_2019.pickle"
    pre, post = get_optimal_sampling_rates(filename1, filename2, M * 1024 * 8 // b, n)
    print("The optimal pre-sampling rates are ", pre)
    print("The optimal post-sampling rates are ", post)
    return pre, post
...
def get_opt_caida_2016_zipf(M, n, alpha, gamma):
    ...
    sum_spread_dict = defaultdict(int)
    real_spread_filename = "../runtime_dataset/records/real_spreads_caida_2016.pickle"
...
def get_opt_caida_2019_zipf(M, n, alpha, gamma):
    ...
    units = M * 1024 * 8 // (int(math.log2(n)) + 1)
    sum_spread_dict = defaultdict(int)
    real_spread_filename = "../runtime_dataset/records/real_spreads_caida_2019.pickle"
    f = open(real_spread_filename, 'rb')
    real_spreads = pickle.load(f)
    flow_p_table, num_dict, p_dict = zipf(alpha, gamma, filename, dir_name)
    for i in range(1, n + 1):
...
if __name__ == '__main__':
    #print(get_opt_caida_2019_zipf(100, 3, 3.0, 3))
    parser = argparse.ArgumentParser(description='generation')
    parser.add_argument('--func_name', type = str, required = True, help = 'function name')
    parser.add_argument("--dataset", type = int, required = True, help = "2016 or 2019")
    parser.add_argument("--mem_kb", type = int, nargs = "+", required = False, help = "on-chip memory (KB)")
    parser.add_argument("--n", type = int, required = False, help = "The supported maximal priorities")
    args = parser.parse_args()
    if args.func_name == "generate":
        if args.dataset == 2016:
            generate_caida_2016_uniform(args.n)
        elif args.dataset == 2019:
            generate_caida_2019_uniform(args.n)
    elif args.func_name == "opt":
        pre_lst = []
        post_lst = []
        filename = "../caida_" + str(args.dataset) + "_" + str(args.n) + "_.txt"
        for M in args.mem_kb:
            if args.dataset == 2016:
                pre, post = get_opt_caida_2016(M, args.n)
                pre_lst.append(pre)
                post_lst.append(post)
            elif args.dataset == 2019:
                pre, post = get_opt_caida_2019(M, args.n)
                pre_lst.append(pre)
                post_lst.append(post)
        f = open(filename, 'w')
        for i in range(len(pre_lst)):
            f.write(str(pre_lst[i]) + "\n")
            f.write(str(post_lst[i]) + "\n")
            f.write("==================\n")
        f.close()